# YouTube Command Palette Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a Cmd+K command palette for YouTube that feels native to the site, with video controls, navigation, and context-aware actions.

**Architecture:** Single Tampermonkey userscript with modular structure. Core palette logic (keyboard handling, rendering, filtering) is separated from YouTube-specific commands and theming. The palette uses YouTube's dark theme colors and Roboto font to feel native. Context detection scrapes the DOM to determine current page type (home, watch, channel, search) and extracts relevant state (video ID, playback position, etc.).

**Tech Stack:** Tampermonkey userscript, vanilla JS (ES6+), CSS custom properties for theming, MutationObserver for SPA navigation detection.

---

## Task 1: Project Setup and Basic Userscript Shell

**Files:**
- Create: `keyring-youtube.user.js`
- Create: `README.md`
- Create: `.gitignore`
...
```bash
cd /Users/user1/projects/keyring
git init
```
...
**Step 4: Create userscript shell**
...
```bash
git add .
git commit -m "chore: initial project setup with userscript shell"
```

---

## Task 2: YouTube-Themed CSS

**Files:**
- Modify: `keyring-youtube.user.js`

**Step 1: Add CSS constants after 'use strict'**
...
**Step 2: Verify syntax by checking in browser console**

Open YouTube in browser, paste the CSS variable into console to ensure no syntax errors.

**Step 3: Commit**

```bash
git add keyring-youtube.user.js
git commit -m "feat: add YouTube-themed CSS for command palette"
```

---

## Task 3: Core Palette UI and State Management

**Files:**
- Modify: `keyring-youtube.user.js`

**Step 1: Add utility functions after CSS constant**
...
**Step 3: Add UI creation functions**
...
**Step 4: Add open/close functions**
...
```bash
git add keyring-youtube.user.js
git commit -m "feat: add core palette UI rendering and state management"
```

---

## Task 4: Input Handling and Keyboard Navigation

**Files:**
- Modify: `keyring-youtube.user.js`

**Step 1: Add input handler functions (after open/close section)**
...
**Step 2: Add global keyboard handler (before the closing IIFE)**
...
```bash
git add keyring-youtube.user.js
git commit -m "feat: add input handling and global keyboard shortcuts"
```

---

## Task 5: YouTube Context Detection

**Files:**
- Modify: `keyring-youtube.user.js`

**Step 1: Add page type detection (after utilities section)**
...
**Step 2: Add timestamp formatting utility**
...
```bash
git add keyring-youtube.user.js
git commit -m "feat: add YouTube context detection for videos and channels"
```

---

## Task 6: Video Playback Commands

**Files:**
- Modify: `keyring-youtube.user.js`

**Step 1: Add video control functions (after context detection)**
...
```bash
git add keyring-youtube.user.js
git commit -m "feat: add video playback control functions"
```

---

## Task 7: Navigation and Copy Commands
...
```bash
git add keyring-youtube.user.js
git commit -m "feat: add navigation and copy functions"
```

---

## Task 8: Commands List and Key Sequences

**Files:**
- Modify: `keyring-youtube.user.js`

**Step 1: Add getCommands function (after copy functions)**
...
**Step 2: Add getKeySequences function**
...
```bash
git add keyring-youtube.user.js
git commit -m "feat: add commands list and vim-style key sequences"
```

---

## Task 9: SPA Navigation Handling

**Files:**
- Modify: `keyring-youtube.user.js`

**Step 1: Add SPA navigation observer (before the closing IIFE, after keyboard handler)**
...
```bash
git add keyring-youtube.user.js
git commit -m "feat: add SPA navigation detection for YouTube"
```
...
## Task 11: Final Assembly and Cleanup

**Files:**
- Modify: `keyring-youtube.user.js`

**Step 1: Verify complete script structure**

The final `keyring-youtube.user.js` should have this structure (verify all sections present):
...
**Step 2: Remove debug console.log (optional, or keep for now)**

**Step 3: Final commit**

```bash
git add -A
git commit -m "feat: complete YouTube command palette v0.1.0"
```
...
|------|-------------|-----------|
| 1 | Project setup, shell | 5 min |
| 2 | YouTube-themed CSS | 10 min |
| 3 | Core palette UI | 10 min |
| 4 | Input and keyboard handling | 10 min |
| 5 | YouTube context detection | 15 min |
| 6 | Video playback commands | 10 min |
| 7 | Navigation and copy commands | 10 min |
| 8 | Commands list and sequences | 10 min |
| 9 | SPA navigation handling | 5 min |
| 10 | Testing checklist | 5 min |
| 11 | Final assembly | 5 min |

**Total: ~95 minutes**

## Future Enhancements (not in this plan)

- Playlist controls (next/prev video)
- "Watch Later" / "Save to playlist" actions
- Transcript search/copy
- Comment navigation
- Search YouTube from palette
- Recently watched videos quick access
