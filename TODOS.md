...
3. [google] add support for image search page (grid view with copy paste)
   ...
   - ✅ **Iteration 3**: Copy image to clipboard with `yy`. Fetch selected image blob, use `navigator.clipboard.write()` with `ClipboardItem`. Show success/failure message. Parses full-size image URLs from Google's inline script metadata (keyed by `data-docid`), which is the same URL that loads in the preview panel. (v0.5.95)
4. ✅ [google] add copying of links (yy) — `yy` on search page copies selected link URL to clipboard via `copyToClipboard`. Shares the `getSelectedItem` callback plumbing with image copy. (v0.5.95)
