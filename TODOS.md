...
3. [google] add support for image search page (grid view with copy paste)
   ...
   - ✅ **Iteration 3**: Copy image to clipboard with `yy`. Fetch selected image blob, use `navigator.clipboard.write()` with `ClipboardItem`. Show success/failure message. Scrapes full-size HTTP image URL (not small data-URI thumbnail) from `[data-lpage]` elements. Extension already has `clipboardWrite` permission. (v0.5.94)
4. ✅ [google] add copying of links (yy) — `yy` on search page copies selected link URL to clipboard via `copyToClipboard`. Shares the `getSelectedItem` callback plumbing with image copy. (v0.5.94)
